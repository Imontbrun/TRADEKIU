...
**Contexto operativo (confirmado por el usuario):** mercados de forex/CFD. Zona horaria America/Caracas (UTC-4). La sesión arranca con el NY open (9:30 AM ET). La semana de trading es lunes–viernes; los fines de semana no se opera.
...
- **FR-8** De cada transcripción la IA extrae un registro estructurado del día con esquema fijo: estado emocional predominante (escala 1–5 + etiquetas, ej. "frustrado", "eufórico", "neutro"), disciplina (¿siguió su plan?: sí / parcial / no), errores señalados (lista), aciertos señalados (lista), razón de inacción (si no tradeó), y un resumen de 2–3 frases. El extracto queda asociado al día; la transcripción completa se conserva como respaldo.
...
- **FR-14** El dashboard muestra, para el período seleccionado (selector: semana actual / mes actual / todo el historial): P&L (por sesión y acumulado), win rate, R:R promedio y su evolución en el tiempo (promedio móvil — ver si el R:R mejora o empeora semana a semana), y número de trades por sesión.
...
- **FR-19** Los reportes quedan archivados y navegables, y cada reporte nuevo contrasta contra la historia acumulada: señala qué cambió respecto a semanas anteriores y si los hábitos detectados antes persisten o se corrigieron. El reporte debe profundizar a medida que crece la historia — de describir (semanas 1–2) a correlacionar (mes 1) a funcionar como espejo de la evolución del trader (mes 2+). Nunca una plantilla estática.
...
| Ritual del lunes | El reporte semanal se vuelve parte de la preparación previa al open | Reportes tan largos o genéricos que dejan de leerse |
...
### Supuestos vigentes (inferidos por el PM, razonables pero no confirmados explícitamente)

| Dónde | Supuesto |
|---|---|
| FR-7 | Audio en español con vocabulario de trading en inglés mezclado |
| FR-10 | v1 solo soporta el CSV de TradingView |
| FR-18 | Umbral mínimo de 3 días con datos para emitir correlaciones |
| FR-20 | Lazy generation del reporte si la app corre localmente |
| NFR-5 | Protección simple (clave única) si se despliega accesible por internet |
